...
class z_stack_window_object(Experiment, QtWidgets.QWidget, UiTools):
    ...
    def __init__(self, activeDatafile, \
                 ...
                 stage_enabled, threadCount, pool, \
                 ui_file = os.path.join(os.path.dirname(__file__),'z_stack_window.ui'),   parent=None):
...
#        self.stack_plot.clear()
        ...
        print(' ' )
        print('BEGIN Z STACK')
        self.SpectraStack_display.canvas.ax.cla()
...
#                self.stack_plot.plot(self.wl_in_nm, self.current_processed_spec)
        ...
        """ Save into txt file """
        filename = 'StackedDF_zStep' + str(self.stack_step_size) + '_zValues' + str(self.stack_z_values) \
            + '_' + str(self.description_LineEdit.text().strip())
        if filename == 'Description...':
            filename == 'StackedDF_NP' + str(self.current_temp) + 'K_'+ \
                    str(self.current_field) + 'T_' + \
                    str(self.current_z_position) + 'mm'
