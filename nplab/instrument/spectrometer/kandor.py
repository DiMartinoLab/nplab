...
class Kandor(Andor):
    ...
    def __init__(self, pixel_number=1024,
        ...
        self.ImageFlip = 0
#old get axis - was giving weird value:    
...
#        
#        return X  
#end   , new version below: 
    def get_x_axis(self):
        if self.use_shifts: # Converts to Raman Shift (or Wavenumber)
            wavelengths = np.array(self.kymera.GetCalibration()[::-1])
            return ( 1./(laser_wl*1e-9)- 1./(wavelengths*1e-9))/100    
        else:
            return self.kymera.GetCalibration()[::-1]
    x_axis = property(get_x_axis)#x_axis=NotifiedProperty(get_xaxis) #This is grabbed by the Andor code 
#    x_axis=NotifiedProperty(get_x_axis) 
